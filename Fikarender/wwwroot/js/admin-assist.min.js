...
/*function submitDownload() {
    ...
    UIkit.modal('#download-modal').hide();
    $('#preloader').fadeOut(100);
}*/
function downloadCvFile() {
    $('#preloader').fadeIn();
    $('#download-form').submit();
    var assistId = $('#download-form input').val();
    $.post('/admin/download-cv-file', { 'assistId': assistId },
        function (response) {
            if (response === false) {
                UIkit.notification({ message: 'چنین فایلی برای دانلود یافت نشد.', status: 'danger', pos: 'bottom-center', timeout: 5000 });
            }
        }, 'json').then(function () {
            $('#preloader').fadeOut();
        });
