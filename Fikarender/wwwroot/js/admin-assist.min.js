...
function addTag(n) {
    var t = $(n).val(), i; t > 0 && (preloader(), i = $("#prdId").val(), $.post("/admin/add-product-tag", { tagId: t, productId: i }, function (t) {
        if (t > 0) {
            var i = $("option:selected", n).text();
            $("#currentTags ul").append('<li><span class="far fa-times-circle pointer" data-id="' + t + '" onclick="removeTag(this)" ><\/span><span class="xs uk-margin-small-right">' + i + "<\/span><\/li>"); $("option:selected", n).remove()
